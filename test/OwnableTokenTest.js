const XAG = artifacts.require('ArgentXAG.sol');
...
// Test that XAG operates correctly as an Ownable token.
contract('Ownable XAG', function ([_, admin, anotherAccount, anotherAccount2, owner]) {
  beforeEach(async function () {
    const xag = await XAG.new({from: owner});
  ...
  describe('as an initializable token', function () {
    ...
    it('constructor initializes the implementation contract and pauses it to avoid misleading state there', async function () {
      const isPaused = await this.xag.paused();
      assert.strictEqual(isPaused, true);
      const currentOwner = this.xag.owner();
      assert.notStrictEqual(currentOwner, ZERO_ADDRESS);
      await assertRevert(this.xag.initialize());
    });
  });
});
